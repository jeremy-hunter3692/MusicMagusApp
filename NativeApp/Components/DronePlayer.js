...
const DronePlayer = () => {
  ...
  // console.log('not use', droneAudioSrc, dronePlaying)
  useEffect(() => {
    // When either source or playing flag changes, decide what to do
    const rootValue = droneAudioSrc || null
    const effectRequestId = ++requestIdRef.current
    // console.log('use', droneAudioSrc, dronePlaying, effectRequestId)
    ...
    if (dronePlaying) {
      if (!rootValue) {
        // nothing to play
      } else if (srcRef.current !== rootValue) {
        // new source: stop existing then load new
        const myId = ++requestIdRef.current
        stopAndCleanup()
          .then(() => loadAndPlay(rootValue, myId))
          .catch(() => loadAndPlay(rootValue, myId))
      } else {
